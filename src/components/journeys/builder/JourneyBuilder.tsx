...
export default function JourneyBuilder() {
  ...
  const updateStage = (index: number, touchpoints: any[]) => {
    setForm(prev => ({
      ...prev,
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {currentStep > 0 && currentStep < 4 && (
              <StageForm
                stageName={form.stages[currentStep - 1].name}
                touchpoints={form.stages[currentStep - 1].touchpoints}
                onUpdate={(touchpoints) => updateStage(currentStep - 1, touchpoints)}
              />
            )}
